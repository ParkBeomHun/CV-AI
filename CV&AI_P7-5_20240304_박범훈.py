...
dmlp.compile(loss='categorical_crossentropy',optimizer = Adam(learning_rate=0.0001),metrics=['accuracy'])   #학습 전에 학습 과정 설정
hist = dmlp.fit(x_train,y_train,batch_size = 128,epochs=50,validation_data=(x_test,y_test),verbose=2)       #실제로 학습 하는 과정
print('정확률=',dmlp.evaluate(x_test,y_test,verbose=0)[1]*100)

dmlp.save('dmlp.trained.h5')
